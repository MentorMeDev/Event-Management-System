...
function RoleLayout({ role = "user", title = "Dashboard", children }) {
  const roleConfig = {
    user: {
      panelName: "User Portal",
      menuItems: [
        { label: "Dashboard", path: "/user/dashboard", icon: LayoutDashboard },
        { label: "Events", path: "/user/events", icon: CalendarDays },
        { label: "Registrations", path: "/user/registrations", icon: ClipboardCheck },
        { label: "Tickets", path: "/user/tickets", icon: Ticket },
        { label: "Profile", path: "/user/profile", icon: User },
      ],
    },
    staff: {
      panelName: "Staff Portal",
      menuItems: [
        { label: "Dashboard", path: "/staff/dashboard", icon: LayoutDashboard },
        { label: "Assigned Events", path: "/staff/assigned-events", icon: CalendarDays },
        { label: "Tasks", path: "/staff/tasks", icon: ClipboardCheck },
        { label: "Check-In", path: "/staff/check-in", icon: CheckCircle2 },
        ...
        <aside className="hidden w-72 border-r border-white/10 bg-white/10 backdrop-blur-2xl lg:block">
          <div className="flex h-full flex-col px-6 py-8">
            <div className="mb-10">
              ...
              <p className="mt-1 text-sm text-slate-300">{currentRoleConfig.panelName}</p>
            </div>
            ...
            <div className="mt-auto rounded-3xl border border-white/10 bg-white/10 p-5 backdrop-blur-xl">
              <p className="text-sm text-slate-300">Engagement</p>
              <h3 className="mt-2 text-3xl font-bold text-cyan-300">96.2%</h3>
              <p className="mt-2 text-sm text-slate-300">
                Real-time interactions and updates are healthy.
              </p>
            </div>
          </div>
        </aside>
        ...
        <div className="flex-1">
          <header className="border-b border-white/10 bg-white/5 px-6 py-5 backdrop-blur-2xl">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-sm text-slate-300">{currentRoleConfig.panelName}</p>
